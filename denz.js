...
- Hari : ${week} ${weton}
- Tanggal : ${calender}`
sendButLocation(from, `${menu}`, "F A I Z Z 右", {jpegThumbnail:gambar,name:""}, [
{buttonId:`allmenu`,buttonText:{displayText:'📑SHOW MENU'},type:1},
{buttonId:`owner`,buttonText:{displayText:'👑OWNER'},type:1},
{buttonId:`script`,buttonText:{displayText:'🔖SCRIPT'},type:1}
...
└─────────────────❒`
sendButLocation(from, `${menu}`, "F A I Z Z 右", {jpegThumbnail:gambar,name:""}, [{buttonId:`owner`,buttonText:{displayText:'👑OWNER'},type:1},{buttonId:`script`,buttonText:{displayText:'📒SCRIPT'},type:1}], {contextInfo: { mentionedJid: [otod]}})
break
case 'command':
 stod = `${sender}`
 listMsg = {
              ...
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							denz.sendMessage(_.jid, buff, image, { viewOnce:false, caption: `${body.slice(4)}`})
						}
						...
						buff = await denz.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							denz.sendMessage(_.jid, buff, video, { viewOnce:false, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
...
└ ${prefix}ytmp4 [link yt] = Video`})
			...
			case 'ping':
				...
				case 'liststicker':
                   ...
                   case 'donasi':
case 'donate':
reply(`Halo ${pushname}! Metode Pembayaran yg di dukung fzxbotz :\nSAWERIA: saweria.co/faizfzx\nTRAKTIR: trakteer.id/fzxbotz\nPULSA : 081776677125\nGOPAY : 085793887010\nDonasi Yok Kakk! Biar bot bisa on terus 😄`)
break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
